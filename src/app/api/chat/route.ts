...
- 50% 이상 커버 가능하면 __NEEDS_FULL__ 없이 3000자 이내로만 작성하세요.`
    : `[절대 규칙 — 기본 답변]
- 반드시 500자 이내로 작성하세요. 500자를 절대 초과하지 마세요.
- 답변은 반드시 완결된 문장으로 끝나야 합니다. 절대로 단어나 문장 중간에서 잘리면 안 됩니다.
- 글자수 제한에 걸릴 것 같으면 핵심만 압축해서 완전한 내용으로 마무리하세요.
- 질문의 핵심 내용이 반드시 포함되어야 합니다.
- 자세한 답변 전체 내용을 요약하는 것처럼 보여야 합니다. 자세한 답변의 일부처럼 보이게 하지 마세요.
- 헤더(#), 목록(-, •), 표 등 구조가 도움된다면 사용 가능하지만 간결하게 유지하세요.`;
